  ...
  return (
    <div className="relative group">
      ...
      <div className="border rounded-xl p-6 bg-white shadow-sm hover:shadow-md transition-shadow duration-200">
        ...
        <div className={`absolute top-4 right-4 w-12 h-12 rounded-full ${colors.bg} bg-opacity-10 flex items-center justify-center`}>
        ...
        {onDelete && (
          <button
            ...
            className="absolute top-2 right-2 p-1.5 rounded-full text-gray-400 hover:text-red-600 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-opacity duration-200"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">${card.annualFee}/year</p>
          </div>
            ...
            {Object.entries(card.rewardRates)
              ...
              .map(([category, rate]) => (
                <div key={category} className="flex items-center text-sm">
                  <div className={`w-1.5 h-1.5 rounded-full ${colors.accent} mr-2`}></div>
                  <span className="text-gray-600 capitalize">{category}</span>
                  <span className={`ml-auto font-medium ${colors.text}`}>{rate}%</span>
                </div>
              ))}
          </div>
